...
# bush_f1 = [0.17349324496336768,0.11594090930374117,0.050309804550932875,0.]
# williams_f1 = [0.24524756852343063,0.15102974828375287,0.07017543859649122,0.]
# pickle.dump((bush_f1), open('bush.pkl', 'wb'))
# pickle.dump((williams_f1), open('williams.pkl', 'wb'))
...
bush_SVC_f1_mean = {'kernel': 'temp', 'C': 0, 'n_components': 0, 'degree': 0, 'f1': 0.643, 'white': False, 'gamma': 0, 'svd_solver': 'temp'}
williams_SVC_f1_mean = {'kernel': 'linear', 'C': 1, 'n_components': 52, 'degree': 0.517, 'f1': 0.03508771929824561, 'white': True, 'gamma': 0, 'svd_solver': 'full'}

for i in range(1300,limit,350):
    file = open("hey.txt", "a") 
    print("Component: ",i)
    for j in range(len(svd_solver)):
        for z in range(len(white)):
